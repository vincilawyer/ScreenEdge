...
3. Choose independent appearances for extended displays and Universal Control under **边缘样式**: five gradient presets or a custom two-color gradient, or solid color; each has its own opacity. The shared line width is on the same page. These 1.5.0 source features are not included in the v1.3.0 archive.
...
- New installs use subtle white/blue translucent solid colors; older gradient preferences stay unchanged. The retired glass option migrates to solid color, retaining its tint and opacity. Colors are saved independently on each Mac; matching gradients require matching selections. See [appearance notes](docs/appearance.md).
