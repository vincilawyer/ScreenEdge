...
在 [Releases](https://github.com/vincilawyer/ScreenEdge/releases/latest) 下载 `ScreenEdge-1.3.0-macOS-arm64.zip`，解压后把 **跨屏边缘.app** 放入“应用程序”文件夹，再双击打开。预编译版适用于 Apple 芯片（M1 及更新）Mac，要求 macOS 13 或更新版本；Intel Mac 可从源码构建。当前源码为 1.5.0 / 6，包含镜像兼容修复、鼠标所在屏幕模式和独立外观设置；上述 v1.3.0 发布包不含这些更新。未合并时请从 `codex/pointer-screen-20260912` 分支构建，详见 AI 维护说明。
...
6. 设置按“边缘显示、颜色与材质、屏幕与通道、手动标记、锁屏与启动”分组，修改自动保存。关闭设置窗口后工具仍在后台运行；可在“锁屏与启动”关闭“显示菜单栏图标”，边缘提示继续工作。隐藏后重新打开“跨屏边缘.app”即可进入设置、恢复图标；在设置窗口按 ⌘Q 或通过应用菜单的“退出跨屏边缘”退出。登录启动默认关闭，需在设置中自行开启。建议先把应用放到长期保留的位置，再开启登录启动。

## 颜色与材质

在“颜色与材质”页先选择 **扩展屏** 或 **通用控制** 卡片，再编辑该类边缘，立即生效并自动保存：

- **渐变**：海岸、日落、极光、暮紫、月光五组配色，或自定义起始色和结束色。
- **纯色**：七种柔和预设，也可以用系统颜色选择器自由选色。
- **毛玻璃**：原生模糊材质叠加所选颜色，随窗口背后的背景变化。可选预设或自选颜色。
- 每类独立调整 **15%–100% 不透明度**，线条粗细仍为两类共用。开启系统“减少透明度”时，毛玻璃自动改为不透明的柔和实色。

新安装默认霜白 / 雾蓝毛玻璃；已有配置升级保留原蓝绿 / 橙红渐变，直到用户主动调整或恢复默认外观。手动标记沿用通用控制外观。两台 Mac 的设置各自保存，不通过网络同步；若希望两端渐变对应，需要在两台 Mac 选择相同配色。静态示意图展示的是旧版渐变示例。

毛玻璃沿实际通道窄条绘制，不扩大覆盖区域；实际模糊效果取决于背景、粗细和系统合成。设置缩略图是材质示例。详见 [外观实现与验证边界](docs/appearance.md)。
...
- 136 项几何与配置检查（含镜像副屏通道映射与解除镜像后的失效）：扩展屏相邻范围、通用控制四边坐标转换、非零屏幕原点、未知设备与无效数据、渐变方向、旧设置升级、配置保存，以及对端示意的方向、连接点、避让和清除。新增覆盖鼠标所在屏幕筛选、共享边界、屏幕间空隙、隐藏或未知状态、模式迁移与锁屏优先级，以及独立外观保存、旧配色迁移、损坏字段回退和颜色范围。
...
- 1.5.0 新增两类通道的独立材质、配色和不透明度；原生检查覆盖各渐变和自定义色的方向与缩放、纯色采样、毛玻璃层、材质切换、减少透明度回退。浅色、深色与最小窗口的设置视图已检查。
- 登录项未自动开启。
